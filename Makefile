...
LDFLAGS += -X "github.com/lavender98/goinception/mysql.TiDBReleaseVersion=$(shell git describe --tags --dirty)"
LDFLAGS += -X "github.com/lavender98/goinception/util/printer.TiDBBuildTS=$(shell date '+%Y-%m-%d %H:%M:%S')"
LDFLAGS += -X "github.com/lavender98/goinception/util/printer.TiDBGitHash=$(shell git rev-parse HEAD)"
LDFLAGS += -X "github.com/lavender98/goinception/util/printer.TiDBGitBranch=$(shell git rev-parse --abbrev-ref HEAD)"
LDFLAGS += -X "github.com/lavender98/goinception/util/printer.GoVersion=$(shell go version)"

TEST_LDFLAGS =  -X "github.com/lavender98/goinception/config.checkBeforeDropLDFlag=1"
...
ifeq ("$(TRAVIS_COVERAGE)", "1")
	...
	# $(OVERALLS) -project=github.com/lavender98/goinception -covermode=count -ignore='.git,vendor,cmd,docs,LICENSES' || { $(GOFAIL_DISABLE); exit 1; }
	# $(GOVERALLS) -service=travis-ci -coverprofile=overalls.coverprofile || { $(GOFAIL_DISABLE); exit 1; }

	$(OVERALLS) -project=github.com/lavender98/goinception -covermode=count -ignore='.git,vendor,cmd,docs,LICENSES' -concurrency=1 -- -short || { $(GOFAIL_DISABLE); exit 1; }
else

ifeq ("$(API)", "1")
	@echo "Running in native mode (API)."
	@export log_level=error;
	$(GOTEST) -timeout 30m -ldflags '$(TEST_LDFLAGS)' github.com/lavender98/goinception/session -api
else
...
docker:
	...
	v1=$(shell git tag | awk -F'-' '{print $1}' |tail -1) && docker build -t lavender98/goinception:$${v1} . \
	&& docker tag lavender98/goinception:$${v1} lavender98/goinception:latest

docker-push:
	v1=$(shell git tag|tail -1) && docker push lavender98/goinception:$${v1} \
	&& docker push lavender98/goinception:latest
