...
func (s *session) mysqlCheckField(t *TableInfo, field *ast.ColumnDef) {
    ...
    //新增，列名为create_time,update_time时，默认值只能为CURRENT_TIMESTAMP
	if (field.Tp.Tp == mysql.TypeTimestamp || field.Tp.Tp == mysql.TypeDatetime) && (strings.ToLower(field.Name.Name.String()) == "create_time" || strings.ToLower(field.Name.Name.String()) == "update_time") && hasDefaultValue && !IsCurrentTimestampExpr(defaultExpr){
		s.appendErrorNo(ER_INVALID_DEFAULT_CUR, field.Name.Name)
	}
    //新增，列名为create_time时，不能含有on update的语句
	if (field.Tp.Tp == mysql.TypeTimestamp || field.Tp.Tp == mysql.TypeDatetime) && strings.ToLower(field.Name.Name.String()) == "create_time" && hasDefaultValue && hasOnupdate{
		s.appendErrorNo(ER_INVALID_ON_UPDATE_CREATE_TIME, field.Name.Name)
	}
    //新增，列名为update_time时，必须含有on update的语句
	if (field.Tp.Tp == mysql.TypeTimestamp || field.Tp.Tp == mysql.TypeDatetime) && strings.ToLower(field.Name.Name.String()) == "update_time" && hasDefaultValue && !hasOnupdate{
		s.appendErrorNo(ER_INVALID_ON_UPDATE_UPDATE_TIME, field.Name.Name)
	}
	// 检查bit类型的默认值
